...
function renderLoadMoreButton() {
  ...
  document
    ...
    ?.addEventListener("click", renderNextCharacters);
}
...
async function getOwnerInfo(userId) {
  ...
  if (ownerInfoCache.has(userId)) {
    return ownerInfoCache.get(userId);
  }
  ...
  try {
    ...
    if (!userSnap.exists()) {
      ...
      ownerInfoCache.set(userId, ownerInfo);
    ...
    console.error("作者情報読み込みエラー:", error);
      ...
      <div class="character-list-image-wrap">
        ...
        ${
          imageSrc
            ...
            `
            : `
              <div class="character-list-no-image">
                No Image
              </div>
            `
        }
...
function getNewestCharacterIdsByOwner() {
  const newestCharacterIds = new Set();
  ...
  for (const item of allCharacters) {
    ...
    newestCharacterIds.add(item.id);
  }

  return newestCharacterIds;
}
...
async function renderNextCharacters() {
  ...
  const loadMoreArea = getLoadMoreArea();

  loadMoreArea.innerHTML = `
    <p class="character-load-status">読み込み中...</p>
  `;

  const newestCharacterIdsByOwner = getNewestCharacterIdsByOwner();
...
loadCharacters().catch((error) => {
  console.error("キャラ一覧読み込みエラー:", error);
