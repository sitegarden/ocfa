...
async function loadMembers() {
  try {
    ...
    memberManageContent.innerHTML = `
      <section class="card message-card">
        <h2>読み込みに失敗しました</h2>
        <p>${escapeHtml(error.message || "時間を置いてもう一度お試しください。")}</p>
      </section>
    `;
  }
}
...
function renderMembers(members) {
  ...
  memberManageContent.innerHTML = `
    <section class="card">
      ...
      <div class="button-row">
        <a class="primary-link" href="/works/file/?id=${currentWork.id}">
          作品詳細へ戻る
        </a>
      </div>
    </section>
    ...
    card.innerHTML = `
      <div class="member-main">
        ...
        <div class="member-info">
          ...
          <div class="badge-row">
            <span class="badge">${escapeHtml(getStatusLabel(data.status))}</span>
            ${
              data.createdAt
      ...
      <div class="button-row">
        ${
          data.status === "pending"
            ? `
              <button type="button" class="primary-link approve-member-btn" data-member-id="${item.id}">
                承認する
              </button>

              <button type="button" class="primary-link reject-member-btn" data-member-id="${item.id}">
                見送る
              </button>
            `
            : ""
...
async function init(user) {
  if (!user) {
    renderMessage("ログインが必要です", "参加申請を管理するにはログインしてください。");
    return;
  }
  ...
  if (work.data.userId !== user.uid) {
    renderMessage("管理できません", "参加申請を管理できるのは作品オーナーだけです。");
    return;
  }

  if (work.data.workType !== "shared") {
    renderMessage("共有作品ではありません", "参加申請管理は共有作品で使えます。");
    return;
  }
